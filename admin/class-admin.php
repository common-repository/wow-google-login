<?php if ( ! defined( 'ABSPATH' ) ) exit;
	/**
		* Admin Page Class
		*
		* @package     Wow_Facebook_Login_Pro_ADMIN_LINKS
		* @subpackage  
			...
			if( isset($_POST['action'] )) {
				if ($_POST['action'] === 'ua-export') {
					if( wp_verify_nonce($_POST['ua_export_field'],'ua_export_action') && current_user_can('manage_options') ){
						...
						fputcsv($fp, array('Email','First Name', 'Last Name', 'Date'));	
						foreach ( $ids as $id ) {
							$user = get_userdata( $id );
							fputcsv($fp, array($user->user_email, $user->first_name, $user->last_name, $user->user_registered));
						}
						exit;
					}
				}				
